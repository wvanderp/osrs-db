...
import formatJson from "../common/formatJson.js";

const BUILD_DIR = "./build";
const FILES_TO_COPY = ["README.md", "package.json", "package-lock.json"];
...
async function build() {
  ...
  try {
    const generateTypesScript = path.join(
      repoRoot,
      "scripts",
      "generate-types.ts",
    );
  ...
  try {
    ...
    execSync(
      `node --max-old-space-size=8192 "${tscPath}" -p tsconfig.build.json`,
      {
        stdio: "inherit",
        cwd: buildPath,
      },
  ...
  if (!licenseCopied) {
    console.log(
      yellow(
        `   ⚠️  No LICENSE file found (checked: ${OPTIONAL_FILES.join(", ")})`,
      ),
    );
  }
  ...
  await fs.writeFile(packageJsonPath, formatJson(packageJson) + "\n", "utf-8");
  ...
  try {
    const generateSchemasScript = path.join(
      repoRoot,
      "scripts",
      "generate-schemas.ts",
    );
    ...
    console.log(
      green("   ✅ JSON schemas generated and copied to build/schemas"),
    );
  } catch (error) {
    throw new Error(
      `JSON schema generation failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
