...
async function main() {
  ...
  console.log(
    green("🔧 [generate-schemas] Starting JSON schema generation..."),
  );
  ...
  console.log(
    cyan(`📋 [generate-schemas] Found ${schemaFiles.length} Zod schema files`),
  );
  ...
  for (const schemaFile of schemaFiles) {
    ...
    try {
      ...
      await fs.writeFile(outPath, formatJson(jsonSchema));
      console.log(green(`   ✅ Generated ${basename}.schema.json`));
    } catch (error) {
      console.error(
        red(
          `  ❌ Failed to process ${schemaFile}: ${error instanceof Error ? error.message : String(error)}`,
        ),
      );
    }
  }
