...
function parseJsonFile<T extends JsonObject = JsonObject>(filePath: string): T {
  ...
  try {
    ...
    throw new Error(
      `Invalid JSON in ${filePath}: ${msg}. File starts with: "${snippet}"`,
    );
  }
}
...
async function main(): Promise<void> {
  try {
    ...
    fs.writeFileSync(packageJsonPath, formatJson(packageObj) + "\n", "utf8");
