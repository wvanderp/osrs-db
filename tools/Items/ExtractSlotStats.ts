...
const items = JSON.parse(fs.readFileSync(itemsFilePath, "utf-8")) as {
  id: number;
  name: string;
  examine: string;
}[];
...
function writeTitleToID(titleToID: Record<string, number | null>) {
  ...
  const sorted = Object.keys(titleToID)
    .sort()
    .reduce(
      (acc, key) => {
  ...
  fs.writeFileSync(titleToIDFilePath, formatJson(sorted));
}
...
function findIDDecorative(name: string): number | null {
  ...
  let [namePart, decoration, hashtag] =
    name.match(/(.+)(?:(?: \((.*)\))|(?:#(.*)))/)?.slice(1) ?? [];
  ...
  const id = items.find((item) => {
    return (
      item.name.toLowerCase() === namePart.toLowerCase() &&
      item.examine.toLowerCase().includes(decoration.toLowerCase())
    );
  })?.id;
...
function rewriteName(name: string): string {
  ...
  const spaceNeeders = ["Abyssal dagger", "Bone dagger", "Viggora's chainmace"];
...
export default async function extractSlotStats() {
  for (const [url, slot] of slotPages) {
    ...
    for (let row of rows.slice(1)) {
      ...
      const members =
        columns[2].querySelector("img")?.getAttribute("alt") === "Members";
  ...
  if (skipped.length > 0) {
    ...
    const examples = skipped
      ...
      .map(
        (s) =>
          `${s.reason}:${s.slot}:${s.name}${s.id !== null ? ` [id=${s.id}]` : ""}`,
      );
    console.log(
      `Extracted ${slotStats.length} rows; removed ${skipped.length} duplicate rows; keeping ${unique.length}.`,
    );
  ...
  fs.writeFileSync("./data/slotStats.g.json", formatJson(unique));
}
